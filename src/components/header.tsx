...
import type { MonState, VerdictTone } from "../lib/real";
import { computeVerdict, fmtClock, TARGET_SERVICES } from "../lib/real";
...
export function Header({
  ...
  const reachable = state.api === "ready" || state.api === "not_configured" || state.api === "other";
    ...
    <header className="sticky top-0 z-40 border-b border-ink-700/80 bg-ink-950/85 backdrop-blur-md">
      <div className="mx-auto flex max-w-[1480px] items-center gap-4 px-4 py-3 md:px-8">
        <div className="flex items-center gap-3">
          <span className="relative grid h-11 w-11 place-items-center border border-brand-500/40 bg-brand-900 text-brand-400">
            <LogoMark width={26} height={26} />
            <span className="absolute -right-1.5 -bottom-1.5 border border-brand-500/60 bg-ink-950 px-1 font-mono text-[8px] leading-[1.7] font-bold tracking-widest text-brand-400">
              ККТ34
            </span>
          </span>
          <div className="leading-tight">
            ...
            <div className="font-mono text-[9.5px] tracking-[0.1em] text-ink-400 uppercase">
        ...
        <div className="ml-auto flex items-center gap-2.5 md:gap-4">
          <div className="hidden items-center gap-2 border border-brand-500/25 bg-ink-900 px-3 py-1.5 lg:flex">
            <span className="font-mono text-[10px] font-bold tracking-[0.14em] text-brand-400">АТОЛ</span>
            <span className="text-ink-600">|</span>
            <span className="font-mono text-[10px] font-bold tracking-[0.14em] text-chz-400">Честный ЗНАК</span>
          </div>
          ...
          <div className="hidden items-center gap-2 border border-ink-700 bg-ink-850 px-3 py-1.5 lg:flex">
            ...
            <span className="font-mono text-xs text-ink-200">
              ЛМ {state.version ? state.version : "нет связи"}
            </span>
          </div>

          <div className="hidden items-center gap-2 border border-ink-700 bg-ink-850 px-3 py-1.5 sm:flex">
            <Led tone="info" size="h-2 w-2" />
            <span className="font-mono text-xs text-ink-200">{TARGET_SERVICES.length}+ служб</span>
          </div>
...
function Chip({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex min-w-[104px] flex-col gap-0.5 border border-ink-700 bg-ink-900/70 px-3 py-2">
      ...
      <span className="truncate font-mono text-sm font-semibold tabular-nums text-ink-100">{value}</span>
    </div>
  );
}
...
export function VerdictStrip({
  ...
  const apiLabel =
    state.api === "ready"
      ...
      : state.api === "not_configured"
        ? "NOT CONFIG"
        : state.api === "other"
          ? (state.apiHttpStatus !== null ? `HTTP ${state.apiHttpStatus}` : "ДРУГОЙ")
  ...
  const progress = state.lastPoll ? Math.min(100, (elapsed / state.interval) * 100) : 0;
  const secsLeft = state.lastPoll ? Math.max(0, Math.ceil((state.interval - elapsed) / 1000)) : null;
    ...
    <section className="panel animate-rise relative overflow-hidden" style={{ animationDelay: "60ms" }}>
      ...
      <div className="flex flex-col gap-6 p-5 md:flex-row md:items-end md:justify-between md:p-7">
        <div className="min-w-0">
          ...
          <p className="mt-3 max-w-xl font-mono text-xs leading-relaxed text-ink-300 md:text-[13px]">
            {verdict.detail}
          </p>
        </div>
        ...
        <div className="flex flex-col gap-4">
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
            <Chip label="Службы" value={`${TARGET_SERVICES.length} · реестр`} />
            <Chip label="API :5995" value={apiLabel} />
            <Chip label="Отклик" value={ping !== null ? `${ping} мс` : "—"} />
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="mt-1.5 font-mono text-[10.5px] tracking-wide text-ink-400">
                {state.lastPoll
                  ? `автоопрос через ${secsLeft} с · интервал ${state.interval / 1000} с`
